...
public class ClientResDecoder implements Decoder {
    ...
    static {
        objectMapper = (new ObjectMapper()).enable(SerializationFeature.INDENT_OUTPUT).enable(
                DeserializationFeature.FAIL_ON_IGNORED_PROPERTIES)
            .enable(DeserializationFeature.FAIL_ON_NULL_FOR_PRIMITIVES)
            .disable(DeserializationFeature.ACCEPT_FLOAT_AS_INT)
            .disable(DeserializationFeature.FAIL_ON_UNKNOWN_PROPERTIES).setSerializationInclusion(
    ...
    public Object decode(Response response, Type type)
        ...
        if (response.status() == HttpStatus.OK.value()) {
            ...
            Result result = objectMapper.readValue(response.body().asInputStream(),
                new TypeReference<>() {
                });
            ...
            if (result.getCode() == 200) {
                if (type instanceof ParameterizedType) {
                    ...
                    if (pt.getRawType().equals(Result.class) || pt.getRawType()
                        .equals(ResultList.class)) {
