...
public class DependencyParser {
    ...
    public static List<DependencyCoordinate> getDependencyCoordinate(){
        ...
        Set<String> collect = dependencies.stream().map(it -> it.getArtifactId())
            .collect(Collectors.toSet());
        //2.过滤后组装DependencyCoordinates
        dependencyList.stream().filter(it->it.getFirst().contains("tech.blacklake")&&collect.contains(it.getMiddle())&&(!ignoreGroupIds.contains(it.getFirst()))).forEach(
            it->
                dependencyCoordinates.add(new DependencyCoordinate(
                    it.getFirst(),it.getMiddle(),it.getLast()
                ))
        );
    ...
    private static void parseParentProjectPomXml(){
        ...
        try {
            Document document = saxReader.read(new File(
                StringUtil.concatPath(Constant.PROJECT_ROOT_PATH, "pom.xml")));
            ...
            List<Element> properties = secondaryElements.stream()
                .filter(it -> it.getQName().getName().equals("properties")).collect(
                    Collectors.toList()).get(0).elements();
            for (Element property : properties) {
                versionMap.put(property.getQName().getName(), property.getStringValue());
            }
            //4.装配父工程pom文件中依赖集合
            List<Element> dependencies = secondaryElements.stream()
                .filter(it -> it.getQName().getName().equals("dependencyManagement")).collect(
                    Collectors.toList()).get(0).elements().stream()
                .filter(it -> it.getQName().getName().equals("dependencies")).collect(
                    Collectors.toList()).get(0).elements();
            for (Element dependency : dependencies) {
                ...
                dependencyTriple.setMiddle(
                    dependency.elements().get(1).getStringValue());//artifactId
    ...
    private static void parseCurrentProjectPomXml(){
        ...
        try {
            ...
            FileInputStream fis = new FileInputStream(new File(StringUtil.concatPath(Constant.PROJECT_ROOT_PATH,Constant.CODE_GENERATOR_PROJECT_NAME,"pom.xml")));
